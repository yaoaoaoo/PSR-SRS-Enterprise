...
All implementations are self-contained.  Accept any iterable of objects
satisfying ``RankedItemProtocol`` (i.e. having ``.item_id`` and ``.score``).
...
def _ideal_rels(qrels_for_query: dict[str, int], top_k: int) -> list[int]:
...
def evaluate_query(
    results: Sequence[RankedItemProtocol],
    query_id: str,
    query_text: str,
    ...
    Args:
        results: Ranked search results for this query (any objects with
                 ``.item_id`` and ``.score``).
    ...
    for k in ks:
        ...
        # ---- Recall@K ----
        metric.recall[k] = (
            rel_in_top / total_relevant if total_relevant > 0 else 0.0
        )
        ...
        # ---- NDCG@K ----
        rels = [qrels_for_query.get(r.item_id, 0) for r in top_k]
        dcg = _dcg(rels)
...
def evaluate_all(
    all_results: dict[str, list[RankedItemProtocol]],
    ...
    Args:
        ...
        queries: List of query dicts (each has ``query_id``, ``query_text``).
        qrels: Qrels dict ``{qid: {iid: grade}}``.
        ks: K cutoffs.
    ...
    for qid in sorted(all_results.keys()):
        ...
        m = evaluate_query(results, qid, text, qrels, ks, relevance_threshold)
        metrics.append(m)
