"""Personalization evaluation — behavior metrics + qrels metrics + coverage.

Identical algorithms to PSR-SRS-MVP.
"""
...
def compute_behavior_metrics(
    ...
    for k in ks:
        ...
        metrics[f"positive_recall_at_{k}"] = (
            pos_in_top / len(positive_items) if positive_items else 0.0
        )
...
def compute_qrels_metrics(
    ...
    for k in ks:
        ...
        metrics[f"recall_at_{k}"] = (
            rel / total_relevant if total_relevant > 0 else 0.0
        )
...
def compute_candidate_coverage(
    ...
    for rid in eligible_rids:
        info = test_requests.get(rid)
        if info is None:
            continue  # eligible rid not found in test_requests — skip
        qid = info["query_id"]
    ...
    return {
        ...
        "request_level_candidate_positive_coverage": (
            round(covered_req / len(eligible_rids), 6) if eligible_rids else 0.0
        ),
        ...
        "item_level_candidate_positive_recall": (
            round(covered_pos_items / total_pos_items, 6) if total_pos_items else 0.0
        ),
        "warm_request_coverage": (
            round(warm_covered / warm_total, 6) if warm_total else 0.0
        ),
        "fallback_request_coverage": (
            round(cold_covered / cold_total, 6) if cold_total else 0.0
        ),
    }
